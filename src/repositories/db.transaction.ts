...
export const getTransactions = async ({
  ...
  const formattedStartDate = from ? startOfDay(new Date(from)) : undefined;
  const formattedEndDate = to ? endOfDay(new Date(to)) : undefined;
  ...
  const data = await prisma.$transaction([
    prisma.transaction.count({
      where: {
        ...
        createdAt: {
          ...(from && { gte: formattedStartDate }),
          ...(to && { lte: formattedEndDate }),
        },
        ...(type && { type }),
